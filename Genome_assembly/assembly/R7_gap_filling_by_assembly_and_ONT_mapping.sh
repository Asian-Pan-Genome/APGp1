...
if [ ! -d "${wkdir}/${famID}-01/R4_hifiasm_trio_ont_update" ]; then
	...
	ln -s ${wkdir}/${famID}-01/R4_hifiasm_trio_ont/${famID}_hifiasm_trio_ont.dip.hap1_ctg.fa ${famID}_hifiasmTrio_pat_ctg.fasta
	ln -s ${wkdir}/${famID}-01/R4_hifiasm_trio_ont/${famID}_hifiasm_trio_ont.dip.hap2_ctg.fa ${famID}_hifiasmTrio_mat_ctg.fasta
fi
...
###Gap-filling by assemblies
echo "##PAT from ${famID} gap filling by assemblies starts..."
echo ">> 1. Gap filling by hifiasm HIC assemblies..."
perl ${wkdir}/gfasm.pl ${famID}_hifiasmTrio_pat_ctg.fasta ${famID}_hifiasmHiC_pat_ctg.fasta ${famID}_pat_R4_R5
echo ">> 1. Finished!"

echo ">> 2. Gap filling by verkko assemblies..."
perl ${wkdir}/gfasm.pl ${famID}_pat_R4_R5.paf.fasta ${famID}_verkko_pat_ctg.fasta ${famID}_Pat_R4_R5_R3
echo ">> 2. Finished!"

echo ""
echo "##MAT from ${famID} gap filling by assemblies starts..."
echo ">> 1. Gap filling by hifiasm HIC assemblies..."
perl ${wkdir}/gfasm.pl ${famID}_hifiasmTrio_mat_ctg.fasta ${famID}_hifiasmHiC_mat_ctg.fasta ${famID}_mat_R4_R5
echo ">> 1. Finished!"

echo ">> 2. Gap filling by verkko assemblies..."
perl ${wkdir}/gfasm.pl ${famID}_mat_R4_R5.paf.fasta ${famID}_verkko_mat_ctg.fasta ${famID}_Mat_R4_R5_R3
echo ">> 2. Finished!"
